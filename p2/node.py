...
class Node:
    ...
    def receive_request(self, from_pid, timestamp):
        ...
        if self.in_critical_section:
            print(f"[RPC Client: {self.pid}] ---- Received REQUEST from Node {from_pid} while in CS! Denying access: {self.in_critical_section}")
            return False
        else:
            print(f"[RPC Client: {self.pid}] ---- Received REQUEST from Node {from_pid} (ts={timestamp}), replying {self.in_critical_section}")
            return True
